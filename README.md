...
```
TNFD-disclosure/
├── SKILL.md                        # Agent Skill 定义（139 行，符合 v4 规范）
├── prompts/                        # 分阶段提示词
...
当前 Registry 状态：**12 条 pending**，已通过 `validate_claims.py` 扫描 10 个活跃文件，确认无 pending claim 被确定式输出。
...
```
用户：我是深证 100 成分股，2026 年是不是必须做 TNFD？

TNFD-disclosure：
• 必须：《上市公司可持续发展报告指引》要求 2026-04-30 前发布 2025 年度报告。
• TNFD 本身非强制，但可用于支撑自然/生物多样性议题披露。
• 建议先判断业务是否有自然相关重要议题，再决定 TNFD 对齐深度。

用户：帮我直接生成完整 TNFD 报告。

TNFD-disclosure：
• 当前 evidence gate 未通过，暂不生成"完整报告"。
• 建议按顺序完成：Scope → Benchmark → LEAP → Prepare → Assurance Readiness。
• 请问您目前有哪些数据：资产坐标、供应链分布、ENCORE 数据？
```
